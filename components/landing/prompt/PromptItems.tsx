...
const PromptItems = () => {
    ...
    useEffect(() => {
        const updateHistories = () => {
            const conversationKeys = Object.keys(localStorage).filter((key) => key.startsWith('chat-prompt-'));
    ...
    const onTitleChange = (id: string) => {
        const newTitle = prompt('Enter new title');

        if (newTitle) {
            const history = localStorage.getItem(`histories-${id}`);
            if (history) {
                const historyObj = JSON.parse(history);
                historyObj.title = newTitle;
                localStorage.setItem(`chat-prompt-${id}`, JSON.stringify(historyObj));
    ...
    const onHistoryDelete = (id: string) => {
        localStorage.removeItem(`chat-prompt-${id}`);
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className='flex items-center space-x-1'>
                                ...
                                <button className='block' onClick={() => onTitleChange(pinHistory.id)}>
                                    <TiBrush className='text-lg hover:fill-green-500' />
                                </button>
                                <button className='block' onClick={() => onHistoryDelete(pinHistory.id)}>
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className='flex items-center space-x-1'>
                                ...
                                <button className='block' onClick={() => onTitleChange(history.id)}>
                                    <TiBrush className='text-lg hover:fill-green-500' />
                                </button>
                                <button className='block' onClick={() => onHistoryDelete(history.id)}>
                                    <TiDeleteOutline className='text-lg hover:fill-red-500' />
                                </button>
                            </div>
                        </div>
